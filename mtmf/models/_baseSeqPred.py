...
class _baseSeqPred(nn.Module):
    ...
    def forecast_hf_multi_step(self, inputs, num_of_steps):
        ...
        for step in range(num_of_steps):
            # Convert to tensors for model call
            x_enc_tensor = torch.tensor(x_encoder_in, dtype=torch.float32)
            x_dec_tensor = torch.tensor(x_decoder_in, dtype=torch.float32)
            y_dec_tensor = torch.tensor(y_decoder_in, dtype=torch.float32)
            
            with torch.no_grad():
                x_forecast_multi, _ = self.model(x_enc_tensor, x_dec_tensor, y_dec_tensor)
            
            # Convert back to numpy
            x_forecast = x_forecast_multi[:,-1,:].numpy()
    ...
    def predict_system_one_cycle(self, inputs, x_step):
        ...
        with torch.no_grad():
            _, y_pred = self.model(x_enc_tensor, x_dec_tensor, y_dec_tensor)
